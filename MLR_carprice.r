...
# train_set
model2 = lm(Price ~. -s_Age - s_KM, data = train_set)
...
# p-value 
model4 = lm(Price ~. -Age - KM -HP -MetColor, 
            data = train_set)

summary(model4)

# remove Doors from the model4
model5 = lm(Price ~. -Age - KM -HP -MetColor -Doors, 
            data = train_set)

summary(model5)

# remove Automatic from the model5
model6 = lm(Price ~. -s_Age - s_KM -HP -MetColor -Doors -Automatic,
            data = train_set)
...
R2(predictTest, test_set$Price)#0.8244

# errors
RMSE(predictTest, test_set$Price)#1205.803
# rmse(predictTest, test_set$Price)# from Metrics package
...
# relative importance of various predictors in predicting alcohol
imp = calc.relimp(model7, type = c("lmg"),rela = TRUE) 
